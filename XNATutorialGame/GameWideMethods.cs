...
namespace XNATutorialGame
{
    class GameWideMethods
    {
        ...
        public int randomNumer()//running number gen for 1,2 for the letter from the die.1 = dead. 2 = alive
        {
            int roll;//contains the initial roll of the number gen
            int dieRollUpdate;//contains the updated value
            roll = numberGen.Next(1, 4);//uses the global Random generator of numberGen. Produces either 1, 2 or 3
            dieRollUpdate = roll;
